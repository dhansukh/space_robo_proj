...
class SimpleLidarOdom(Node):
    ...
    def _add_keyframe(self, pts_body: np.ndarray) -> None:
        ...
        """
        # Discard returns beyond the largest possible cave cross-section.
        # Max chamber radius from cave_graph is 7.7 m; 10 m gives margin for
        # diagonal tunnel shots while blocking beams that escape through the entrance
        # and hit surfaces far outside the cave (those cause the outlier halo).
        MAX_BODY_RANGE = 10.0   # metres from sensor; beyond this = discard
        ranges = np.linalg.norm(pts_body, axis=1)
        pts_body = pts_body[ranges < MAX_BODY_RANGE]
        if len(pts_body) == 0:
            return

        # Statistical outlier removal: discard points with no neighbour within 0.6 m.
        # Using counts >= 2 (needs just 1 other nearby point) keeps sparse-LiDAR
        # wall returns while still dropping completely isolated ghost beams.
        if _HAS_SCIPY and len(pts_body) > 20:
            tree = KDTree(pts_body)
            counts = np.array(
                tree.query_ball_point(pts_body, r=0.6, return_length=True))
            pts_body = pts_body[counts >= 2]
            if len(pts_body) == 0:
                return
