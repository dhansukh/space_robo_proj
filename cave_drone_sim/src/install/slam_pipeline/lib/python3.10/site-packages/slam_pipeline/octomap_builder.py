...
class LogOddsVoxelGrid:
    ...
    def get_2d_slice(
        ...
        """
        Project voxels within [z_min, z_max] to a 2-D occupancy grid.
          100  = occupied  (log-odds > 0.4,  ~1 solid hit)
            0  = free      (log-odds < -0.3, ray passed through)
           -1  = unknown   (never observed)
        Including free space makes cave interiors appear white (navigable)
        rather than grey (unknown), giving a clean nav2-compatible map.
        """
        with self._lock:
            relevant = {
                k: v for k, v in self._grid.items()
                if z_min <= k[2] * self.resolution <= z_max
                and (v > 0.4 or v < -0.3)
            }
        ...
        W = ix_max - ix_min + 1
        H = iy_max - iy_min + 1
        grid_2d = np.full((H, W), -1, dtype=np.int8)  # unknown by default

        for (kx, ky, _), v in relevant.items():
            xi = kx - ix_min
            yi = ky - iy_min
            if 0 <= yi < H and 0 <= xi < W:
                if v > 0.4:
                    grid_2d[yi, xi] = 100           # occupied — always wins
                elif grid_2d[yi, xi] != 100:
                    grid_2d[yi, xi] = 0             # free — don't overwrite wall
...
class OctomapBuilder(Node):
    ...
    def _publish_2d_grid(self, stamp: RosTime) -> None:
        ...
        # ── Dilate occupied cells into unknown cells only (not into free space) ──
        # 1 iteration fills 1-pixel gaps between adjacent wall returns without
        # bloating isolated outlier dots into large blobs (2 iterations did that).
        free_mask = grid_2d == 0
        occ_mask  = grid_2d == 100
        if occ_mask.any():
            if _HAS_SCIPY_ND:
                dilated = _scipy_dilate(occ_mask, iterations=1)
            else:
            ...
            # Only expand into unknown cells — preserve navigable free space
            grid_2d[dilated & ~occ_mask & ~free_mask] = 100
