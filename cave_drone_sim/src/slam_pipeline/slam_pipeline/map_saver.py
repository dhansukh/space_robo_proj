...
def save_2d_grid_pgm(grid: OccupancyGrid, out_dir: str, base: str) -> None:
    ...
    free_mask = data == 0    # free space from ray-casting (cave interior)
    occ_mask  = data == 100  # occupied (cave walls)

    # Remove isolated noise blobs (< 8 pixels) that survived the log-odds filter.
    # Don't touch free-space cells — those represent confirmed navigable area.
    if occ_mask.any():
        try:
            from scipy.ndimage import label as _label, binary_dilation as _dilate
            # One more dilation pass to connect wall fragments into continuous lines,
            # applied only into unknown cells (not into free space).
            expanded = _dilate(occ_mask, iterations=1)
            occ_mask  = expanded & ~free_mask
            labeled, n_comp = _label(occ_mask)
            for comp_id in range(1, n_comp + 1):
                if (labeled == comp_id).sum() < 8:
                    occ_mask[labeled == comp_id] = False
        except ImportError:
            pass

    # Standard ROS map_server greyscale encoding:
    #   0   (black)      = occupied
    #   205 (light grey) = free / navigable
    #   128 (mid grey)   = unknown
